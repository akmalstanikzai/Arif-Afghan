...
export default function Sidebar({ user, page, open, onNavigate, onSignOut, busy }) {
  ...
  return (
    <aside dir={direction} className="flex w-full shrink-0 flex-col bg-brand p-5 text-[#ecf1e8] md:sticky md:top-0 md:h-svh md:w-[215px] md:px-3.5 md:pt-6 lg:w-[248px] lg:px-5">
      ...
      <div className="mt-6 mb-4 hidden px-3.5 text-[9px] leading-6 text-[#92ab9a] md:block">{t('بخش‌های کارخانه')}</div>
      <nav id="dashboard-navigation" aria-label={t('فهرست اصلی')} className={`${open ? 'mt-5 grid' : 'hidden'} gap-2 md:mt-0 md:grid md:min-h-0 md:overflow-y-auto`}>
        {navigation.map(item => (
      ...
      <div className={`${open ? 'block' : 'hidden'} mt-auto pt-4 md:block md:pt-4`}>
        <div className="hidden items-center gap-3 px-4 pt-5 pb-8 text-[#bbc9b4] 2xl:flex"><Icon name="leaf" /><p className="text-xs leading-relaxed">{t('کیفیت در هر مرحله.')}<br /><span className="text-[#e3d6b1]">{t('دقت در هر دانه.')}</span></p></div>
        ...
        <button className="flex w-full cursor-pointer items-center gap-3 rounded-md p-3 text-xs text-[#bfd0c3] hover:bg-[#315444] disabled:cursor-wait disabled:opacity-50" onClick={onSignOut} disabled={busy}><Icon name="logout" />{busy ? t('در حال خروج…') : t('خروج')}</button>
      </div>
    </aside>
  );
}
