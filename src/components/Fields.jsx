...
export function Input({ label, hint, value, onChange, type = 'text', placeholder, ...props }) {
  const { t } = useLanguage();
  return <Field label={label} hint={hint}>{id => <input id={id} className={fieldClass} type={type} placeholder={placeholder ? t(placeholder) : undefined} value={value ?? ''} onChange={e => onChange(e.target.value)} {...props} />}</Field>;
}
export function SearchSelect({ label, value, onChange, options, disabled = false }) {
  ...
  return <div className="relative min-w-0"><span id={id} className="mb-2 block text-xs font-semibold">{t(label)}</span><details ref={root} onKeyDown={e => { if (e.key === 'Escape') { root.current.open = false; root.current.querySelector('summary').focus(); } }}>
    <summary aria-labelledby={id} className={`${fieldClass} cursor-pointer list-none ${disabled ? 'pointer-events-none opacity-50' : ''}`}>{selected?.name || t("Select")}<span className="float-end" aria-hidden="true">⌄</span></summary>
    <div className="absolute z-30 mt-1 w-full rounded-lg border border-line bg-white p-2 shadow-lg">
      <input aria-label={`${t("Search in")} ${t(label)}`} placeholder={t("Search…")} className={fieldClass} value={search} onChange={e => setSearch(e.target.value)} />
      <select aria-label={t(label)} className="mt-2 w-full rounded border border-line p-1 text-sm" size={Math.min(6, Math.max(2, choices.length + 1))} value={String(value || '')} onChange={e => { onChange(e.target.value); root.current.open = false; setSearch(''); root.current.querySelector('summary').focus(); }}>
        <option value="">{t("Select")}</option>{choices.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
      </select>
      {!choices.length && <p className="p-2 text-xs text-stone-500">{t("No matches found.")}</p>}
    </div>
  </details></div>;
}
...
export function Notice({ children }) { const { t } = useLanguage(); return children ? <p className="my-3 rounded-lg bg-emerald-50 p-3 text-sm leading-7 text-emerald-800" role="status">{t(children)}</p> : null; }
...
export function Table({ columns, rows, rowKey = 'id', empty = "Nothing has been recorded yet." }) {
  const { t } = useLanguage();
  return <div className="overflow-x-auto rounded-lg border border-line"><table className="w-full min-w-[560px] text-start text-xs"><thead className="bg-surface"><tr>{columns.map(c => <th className="px-4 py-3 font-semibold whitespace-nowrap" key={c.key}>{t(c.label)}</th>)}</tr></thead><tbody>{rows.map(row => <tr key={typeof rowKey === 'function' ? rowKey(row) : row[rowKey]} className="border-t border-line hover:bg-stone-50">{columns.map(c => <td key={c.key} className="px-4 py-3 leading-6">{c.render ? c.render(row) : row[c.key]}</td>)}</tr>)}</tbody></table>{!rows.length && <p className="p-8 text-center text-sm text-stone-500">{t(empty)}</p>}</div>;
}
