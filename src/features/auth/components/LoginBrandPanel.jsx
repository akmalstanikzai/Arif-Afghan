...
export default function LoginBrandPanel() {
  ...
  return (
    <section dir={direction} className="relative flex flex-col overflow-hidden bg-brand p-7 text-[#f6f3df] sm:min-h-svh sm:p-9 lg:px-16 lg:pt-12 lg:pb-8" aria-label={t('کارخانهٔ برنج')}>
      <Brand />
      <div className="relative z-10 my-auto pt-10 pb-3 sm:pt-20 sm:pb-40">
        <span className="text-[10px] font-bold leading-7 text-brand-gold">{t('از دانه تا شکوفایی')}</span>
        ...
        <p className="hidden max-w-80 text-sm leading-[1.9] text-[#becac0] sm:block">{t('فضای اختصاصی برای مدیریت خرید، پروسس، فروش و حساب‌های کارخانهٔ شما.')}</p>
      </div>
      ...
      <div className="relative z-10 hidden gap-2 text-[10px] tracking-wider text-[#bdc8bd] sm:flex sm:flex-col lg:flex-row lg:justify-between">
        <span>{t('ریشه در کیفیت.')}</span><span>{t('آماده برای هر روز.')}</span>
      </div>
    </section>
  );
}
