...
export default function ExpensesPage() {
  ...
  return <div className="space-y-6"><PageHeading eyebrow="Factory expenses paid" title="Daily expenses" />
    <Card title="Daily expense total"><div className="mb-5 max-w-sm"><DateInput label="Date" value={date} onChange={setDate} required /></div><Alert>{error}</Alert>{summary&&<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat label="Individual daily expenses" value={money(summary.daily)} /><Stat label="Salary expense for the day" value={money(summary.salary)} /><Stat label="Electricity expense for the day" value={money(summary.electricity)} /><Stat label="Total expenses for the day" value={money(summary.total)} /></div>}</Card>
