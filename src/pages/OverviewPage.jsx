...
import { money, weight, number, dateLabel, kindLabels } from '../lib/format.js';
...
export default function OverviewPage({ user, onNavigate }) {
  ...
  return <div className="space-y-6"><PageHeading eyebrow="Factory activity overview" title="Overview" />
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat label="Total raw inventory" value={weight(data.raw_stock.reduce((a,r)=>a+Number(r.quantity),0))} /><Stat label="Processed physical inventory" value={weight(sum('physical'))} /><Stat label="Available inventory" value={weight(sum('available'))} /><Stat label="Customer rice in storage" value={weight(sum('reserved'))} /></div>
    ...
    <Card title="Factory processing statistics"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat label="Processing batches" value={number(s.processing_count,0)} /><Stat label="Input weight" value={weight(s.processing_input)} /><Stat label="Produced product" value={weight(s.processing_output)} /><Stat label="Waste" value={weight(s.wastage)} /></div></Card>
    <Card title="Contract processing statistics"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat label="Contract batches" value={number(s.service_count,0)} /><Stat label="Customer input" value={weight(s.service_input)} /><Stat label="Customer output" value={weight(s.service_output)} /><Stat label="Contract waste" value={weight(s.service_wastage)} /></div></Card>
