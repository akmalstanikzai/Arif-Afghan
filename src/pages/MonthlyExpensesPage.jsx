...
const initial=()=>({solar_month:toSolarHijri(today()).slice(0,7),category_id:'electricity',description:'',responsible:'',amount:''});
export default function MonthlyExpensesPage(){
  ...
  return <div className="space-y-6"><PageHeading eyebrow="Fixed expenses spread across every calendar day" title="Monthly expenses" />
    <Card title="Record monthly fixed expense"><form onSubmit={submit} noValidate><fieldset disabled={busy} className="grid items-end gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <label className="text-xs font-semibold">{t('Fixed expense type')}<select className={`${fieldClass} mt-2`} value="electricity" disabled><option value="electricity">{t('Electricity bill')}</option></select></label>
    ...
    <Card title="Monthly fixed expense records"><Table rows={rows.filter(r=>r.category_id==='electricity')} columns={[{key:'solar_month',label:'Solar Hijri month'},{key:'category_id',label:'Type',render:()=>t('Electricity bill')},{key:'amount',label:'Monthly amount',render:r=>money(r.amount)},{key:'daily_amount',label:'Amount per day',render:r=>money(r.daily_amount)},{key:'description',label:'Description'},{key:'responsible',label:'Responsible person'},{key:'actions',label:'Actions',render:r=><button disabled={busy} className={`${secondaryClass} text-red-700`} onClick={()=>remove(r)}>{t('Delete')}</button>}]} /></Card>
  </div>;
}
