...
declare
  ...
  month_total numeric := 0;
  year_total numeric := 0;
  all_time_total numeric := 0;
...
begin
  ...
  from (
    ...
    union all
    select min(public.mill_solar_month_start(m.solar_month)),
           max(public.mill_solar_month_start(m.solar_month) + public.mill_month_days(m.solar_month) - 1)
      from public.mill_monthly_expenses m
  ) bounds;

  first_record := least(first_record, p_date);
  last_record := greatest(last_record, p_date);
  range_start := least(first_record, week_start, month_start, year_start);
  range_end := greatest(last_record, week_start + 6, month_end, year_end);

  for report_day in
    select d::date from generate_series(range_start, range_end, interval '1 day') days(d)
  loop
    summary := public.mill_daily_expense_summary(report_day);
    amount := coalesce((summary ->> 'total')::numeric, 0);
    if report_day = p_date then day_total := amount; end if;
    if report_day between week_start and week_start + 6 then week_total := week_total + amount; end if;
    if report_day between month_start and month_end then
      month_total := month_total + amount;
      day_rows := day_rows || jsonb_build_array(summary);
    end if;
    if report_day between year_start and year_end then year_total := year_total + amount; end if;
    if report_day between first_record and last_record then all_time_total := all_time_total + amount; end if;
  end loop;
